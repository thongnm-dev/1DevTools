...
3. **Phase 3 — Agent/Skill/Prompt** ✅ **Đã làm**: picker agent/provider (đã tái dùng `ai_account_store` từ khi build Workflow). Thêm `skill_store.rs`/`skill_commands.rs` + `prompt_store.rs`/`prompt_commands.rs` (CRUD, cùng pattern `workflow_store`) + `SkillPage.vue` (`/skill`, list card + category badge + copy instructions) + `PromptPage.vue` (`/prompt`, list + phát hiện biến `{{var}}` real-time + copy tăng `usage_count`). Wire lại `WorkflowPage.vue`'s step dialog: `skillOptions`/`promptOptions` giờ lấy từ 2 store thật thay vì rỗng. Route + menu seed cho cả 2 trang đã thêm.
   - Lưu ý bug đã gặp: chuỗi i18n chứa literal `{{variable}}` khiến vue-i18n compile lỗi ("Not allowed nest placeholder") — làm crash cả dialog. Phải viết lại câu chữ tránh literal `{{ }}` trong message string (khác với `route.subtitle` hiển thị qua Vue interpolation thường — an toàn).
4. **Phase 4 — Workflow/Automation** ✅ **Đã làm**:
   - **Testing**: thêm `CommandKind` (`run`/`test`/`build`) vào `DevCommand` (`dev_runner.rs`), heuristic `infer_kind()` tự phân loại command detect được (npm/flutter/maven/gradle/cargo/go/python/dotnet/make/docker), field `kind` chọn tay ở `GitRunnerAddDialog.vue`, hiển thị icon màu theo kind ở `GitRunnerPanel.vue`.
   - **Engine chạy Workflow**: `useWorkflowRunner.ts` (mới) — chạy tuần tự các step của 1 `Workflow` lên 1 `Workspace` mục tiêu: step `runner`/`terminal` mở tab terminal (autoCommand), step `skill`/`prompt` copy nội dung vào clipboard + mở tab terminal, dừng sớm nếu gặp step `is_latest_step`. `WorkflowPage.vue` có nút "Run" (popover chọn workspace) để chạy tay.
   - **Auto-trigger từ git-watch**: thêm `auto_workflow_id: Option<i64>` vào `Workspace`/`UpdateWorkspaceRequest` (backend + frontend). `WorkspacesPage.vue` có Select "Tự động chạy workflow" trong dialog Edit + icon ⚡ đánh dấu workspace đang bật automation; đăng ký listener `onGitRepoChanged` (global, `src/tauri/events.ts`) — mỗi lần file trong 1 workspace có `auto_workflow_id` thay đổi, tự gọi `useWorkflowRunner().runWorkflow(...)` cho workflow đó.
   - Verify bằng Playwright smoke test (mock `__TAURI_INTERNALS__` + event `git-repo-changed`): (a) `auto_workflow_id` persist đúng qua `workspace_update`, (b) workspace có automation tự mở tab terminal khi nhận event đổi file, (c) workspace không bật automation không tự chạy gì khi nhận event. Cả 3 pass, không lỗi console.
