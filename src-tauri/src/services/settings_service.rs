//! Business logic cho module cài đặt người dùng.
//!
//! Đọc/ghi settings từ database:
//! - User profile → bảng `users` (qua stored procedure)
//! - Theme, language, tab_mode → bảng `user_settings`
...
pub async fn save_settings(request: SaveSettingsRequest) -> AppResult<AppSettings> {
    ...
    if theme != "light" && theme != "dark" {
        return Err(AppError::new("Invalid theme (only light or dark accepted)."));
    }

    let language = request.language.trim().to_string();
    if language != "vi" && language != "en" {
        return Err(AppError::new("Invalid language (only vi or en accepted)."));
    }
