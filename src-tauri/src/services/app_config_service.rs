...
use crate::models::app_config::{AppConfigData, ConfigEntry, ConfigSection, SaveAppConfigRequest};
...
pub fn save_app_config(request: SaveAppConfigRequest) -> AppResult<AppConfigData> {
    ...
    ini.write_to_file_opt(&path, write_opt)
        .map_err(|e| AppError::new(format!("Failed to write config.ini: {e}")))?;
