...
import type { GitCommit, GitCommitDetail } from "@/models/git";
...
<template>
  <Dialog
    ...
    modal
    maximizable
    :header="t('git.dialogs.log.title')"
    :style="{ width: '95vw'}"
  ...
  >
    <div class="flex flex-col gap-3">
      ...
      <div class="flex flex-wrap items-end gap-2 rounded-lg border border-divider bg-canvas p-3">
        <div class="flex flex-col gap-1">
          <label class="text-xs font-bold text-muted">{{ t('git.dialogs.log.fromLabel') }}</label>
          <DatePicker
            v-model="filterFrom"
            date-format="yy-mm-dd"
            :placeholder="t('git.dialogs.log.fromPlaceholder')"
            show-icon
            show-button-bar
            class="w-40"
          />
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs font-bold text-muted">{{ t('git.dialogs.log.toLabel') }}</label>
          <DatePicker
            v-model="filterTo"
            date-format="yy-mm-dd"
            :placeholder="t('git.dialogs.log.toPlaceholder')"
            show-icon
            show-button-bar
            class="w-40"
          />
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs font-bold text-muted">{{ t('git.dialogs.log.messageLabel') }}</label>
          <InputText
            v-model="filterMessage"
            :placeholder="t('git.dialogs.log.messagePlaceholder')"
            class="h-8 w-48 text-sm"
            @keydown.enter="search()"
          />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs font-bold text-muted">{{ t('git.dialogs.log.authorLabel') }}</label>
          <InputText
            v-model="filterAuthor"
            :placeholder="t('git.dialogs.log.authorPlaceholder')"
            class="h-8 w-36 text-sm"
            @keydown.enter="search()"
          />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs font-bold text-muted">{{ t('git.dialogs.log.fileLabel') }}</label>
          <InputText
            v-model="filterFile"
            :placeholder="t('git.dialogs.log.filePlaceholder')"
            class="h-8 w-48 text-sm"
            @keydown.enter="search()"
          />
        </div>
        <Button size="small" :loading="loading" @click="search()">
          <i class="pi pi-search mr-1.5" /> {{ t('git.dialogs.log.search') }}
        </Button>
        <Button size="small" outlined severity="secondary" @click="clearFilters">
          <i class="pi pi-filter-slash mr-1.5" /> {{ t('git.dialogs.log.clearFilters') }}
        </Button>
      </div>
      ...
      >
        ...
        <div class="flex min-w-0 flex-1 flex-col overflow-hidden rounded-md border border-divider">
          <template v-if="selectedCommit">
            ...
            <div class="border-b border-divider bg-canvas px-4 py-2.5">
              <div class="flex items-start gap-2">
                ...
                <button
                  ...
                  :title="detailExpanded ? t('git.dialogs.log.collapse') : t('git.dialogs.log.expand')"
              ...
              <p class="mt-1.5 flex flex-wrap items-center gap-2 text-[11px] text-muted">
                ...
                <button
                  ...
                  :title="t('git.dialogs.log.copySha')"
                  @click="git.copyText(selectedCommit!.hash, 'SHA')"
                >
                  <i class="pi pi-copy text-[10px]" /> {{ t('git.dialogs.log.copySha') }}
                </button>
              </p>
            </div>
            <!-- Files -->
            <div v-if="detailLoading" class="p-4 text-sm text-muted">
              <i class="pi pi-spinner pi-spin mr-1.5" /> {{ t('git.dialogs.log.loadingDetail') }}
            </div>
            <div v-else-if="detail" class="min-h-0 flex-1 overflow-y-auto">
              <div class="border-b border-divider bg-canvas px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-muted">
                {{ t('git.dialogs.log.filesChangedCount', { count: detail.files.length }) }}
              </div>
              <div
          ...
          <div v-else class="flex h-full items-center justify-center p-8 text-center text-sm text-muted">
            {{ t('git.dialogs.log.selectCommitToViewDetail') }}
          </div>
        </div>
      </div>
    </div>
    ...
    <template #footer>
      <DialogFooter :cancel-label="t('git.dialogs.log.close')" cancel-icon="pi pi-times" cancel-severity="danger" hide-confirm @cancel="visible = false" />
    </template>
  </Dialog>
</template>
