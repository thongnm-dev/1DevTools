...
import type { GitFileChange, GitGraphCommit } from "@/models/git";
...
<template>
  <Dialog
    ...
    modal
    maximizable
    :header="t('git.dialogs.graph.title')"
    :style="{ width: '1100px' }"
  ...
  >
    <div class="flex flex-col gap-2">
      ...
      <div class="flex items-center gap-2">
        <span class="text-xs text-muted">{{ t('git.dialogs.graph.commitCount', { count: git.graphCommits.value.length }) }}</span>
        <div class="ml-auto flex items-center gap-2">
          <span class="text-xs text-muted">{{ t('git.dialogs.graph.limitLabel') }}</span>
          <Select
            v-model="graphLimit"
      ...
      >
        ...
        <div class="min-w-0 flex-1 overflow-auto rounded-md border border-divider">
          <div v-if="git.graphLoading.value" class="p-8 text-center text-sm text-muted">
            <i class="pi pi-spinner pi-spin mr-1.5" /> {{ t('git.dialogs.graph.loading') }}
          </div>
          <div v-else-if="!git.graphCommits.value.length" class="p-8 text-center text-sm text-muted">
            {{ t('git.dialogs.graph.noCommits') }}
          </div>
        ...
        >
          <div class="border-b border-divider bg-canvas px-3 py-2">
            ...
            <p class="mt-0.5 flex flex-wrap items-center gap-2 text-[11px] text-muted">
              ...
              <button class="rounded p-0.5 hover:text-brand" :title="t('git.dialogs.graph.copySha')" @click="git.copyText(graphSelected.hash, 'SHA')">
          ...
          <div class="border-b border-divider bg-canvas px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-muted">
            {{ t('git.dialogs.graph.filesCount', { count: git.browserFiles.value.length }) }}
          </div>
          ...
          <div class="min-h-0 flex-1 overflow-auto">
            <div v-if="!git.browserDiff.value" class="flex h-full items-center justify-center p-4 text-center text-xs text-muted">
              {{ t('git.dialogs.graph.selectFileToViewDiff') }}
            </div>
            <div v-else-if="git.browserDiff.value.is_binary" class="p-3 text-xs text-muted">{{ t('git.dialogs.graph.binaryFile') }}</div>
