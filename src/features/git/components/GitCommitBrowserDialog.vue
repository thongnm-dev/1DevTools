...
import type { GitCommit, GitFileChange } from "@/models/git";
...
function copySelectedShas() {
  const shas = props.git.browserCommits.value
    ...
    .map((c) => c.hash);
  if (shas.length) props.git.copyText(shas.join("\n"), t("git.dialogs.commitBrowser.shaCount", { count: shas.length }));
}
...
async function copySelectedFiles() {
  ...
  try {
    ...
    if (files.length) await props.git.copyText(files.join("\n"), t("git.dialogs.commitBrowser.fileCount", { count: files.length }));
  } finally {
...
<template>
  <Dialog
    ...
    modal
    maximizable
    :header="t('git.dialogs.commitBrowser.title')"
    ...
    >
      ...
      <div class="flex w-80 shrink-0 flex-col overflow-hidden rounded-md border border-divider">
        <div class="flex items-center gap-2 border-b border-divider bg-canvas px-2 py-1 text-[11px] text-muted">
          <span class="font-bold uppercase tracking-wide">{{ t('git.dialogs.commitBrowser.commits') }}</span>
          <span v-if="browserSelected.size" class="ml-auto text-brand">{{ t('git.dialogs.commitBrowser.selectedCount', { count: browserSelected.size }) }}</span>
        </div>
        <div class="min-h-0 flex-1 overflow-y-auto">
          <div v-if="git.browserLoading.value" class="p-6 text-center text-sm text-muted">
            <i class="pi pi-spinner pi-spin mr-1.5" /> {{ t('git.dialogs.commitBrowser.loading') }}
          </div>
          <div
      ...
      <div class="flex w-56 shrink-0 flex-col overflow-hidden rounded-md border border-divider">
        <div class="border-b border-divider bg-canvas px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-muted">
          {{ t('git.dialogs.commitBrowser.files', { count: git.browserFiles.value.length }) }}
        </div>
        <div class="min-h-0 flex-1 overflow-y-auto">
          <button
      ...
      <div class="min-h-0 flex-1 overflow-auto rounded-md border border-divider">
        <div v-if="!git.browserDiff.value" class="flex h-full items-center justify-center p-6 text-center text-xs text-muted">
          {{ t('git.dialogs.commitBrowser.selectFilePrompt') }}
        </div>
        <div v-else-if="git.browserDiff.value.is_binary" class="p-4 text-xs text-muted">{{ t('git.dialogs.commitBrowser.binaryFile') }}</div>
    ...
    <template #footer>
      <DialogFooter :cancel-label="t('git.dialogs.commitBrowser.close')" cancel-icon="pi pi-times" cancel-severity="warn" hide-confirm @cancel="visible = false">
        <template #extra>
          ...
          >
            <i class="pi pi-file-export mr-1.5" /> {{ t('git.dialogs.commitBrowser.copyChangedFiles') }}
          </Button>
          <Button size="small" :disabled="!browserSelected.size" @click="copySelectedShas">
            <i class="pi pi-copy mr-1.5" /> {{ t('git.dialogs.commitBrowser.copyShas', { count: browserSelected.size }) }}
          </Button>
        </template>
      </DialogFooter>
    </template>
