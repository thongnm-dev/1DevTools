...
function openTagDialog(target?: { hash: string; label: string }) {
  closeCommitMenu();
  tagTarget.value = target ?? { hash: "", label: `HEAD (${t("git.page.currentBranchLabel")})` };
  ...
  <section class="relative flex min-h-0 flex-1 flex-col gap-2 overflow-hidden">
    ...
    <template v-else>
        ...
        <div class="relative">
          ...
          >
            ...
            <span class="truncate font-semibold text-ink">
              {{ git.activeRepo.value?.name ?? t("git.page.selectRepo") }}
            </span>
          ...
          >
            <div class="max-h-64 overflow-y-auto">
              ...
              >
                ...
                <button
                  ...
                  :title="t('git.page.removeFromList')"
            ...
            <div class="mt-1 flex gap-1 border-t border-divider pt-1.5">
              ...
              >
                <i class="pi pi-folder-open text-xs" /> {{ t("git.page.addLocal") }}
              </button>
              <button
                class="flex flex-1 items-center justify-center gap-1.5 rounded-md px-2 py-2 text-xs font-medium text-secondary transition-colors hover:bg-canvas hover:text-brand"
                @click="closeMenus(); cloneDialogVisible = true"
              >
                <i class="pi pi-cloud-download text-xs" /> {{ t("git.page.cloneUrl") }}
              </button>
            </div>
          </div>
        </div>
          ...
          >
            <div class="relative mb-1">
              ...
              <InputText
                ...
                :placeholder="t('git.page.filterBranchPlaceholder')"
            ...
            <div class="mt-1 border-t border-divider pt-1.5">
              ...
              >
                <i class="pi pi-plus text-xs" /> {{ t("git.page.createNewBranch") }}
              </button>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <span class="font-semibold text-amber-800">{{ t("git.page.rebaseInProgress") }}</span>
        <span class="text-amber-700">{{ t("git.page.resolveConflictHint") }}</span>
        <div class="ml-auto flex gap-2">
          <Button size="small" outlined severity="secondary" @click="openConflictDialog">
            <i class="pi pi-wrench mr-1.5 text-xs" /> {{ t("git.page.resolveConflict") }}
          </Button>
          <Button size="small" :disabled="!!git.busyMessage.value" @click="git.rebaseContinue()">
            <i class="pi pi-play mr-1.5 text-xs" /> {{ t("git.page.continue") }}
          </Button>
          <Button size="small" outlined severity="danger" :disabled="!!git.busyMessage.value" @click="git.rebaseAbort()">
            <i class="pi pi-times mr-1.5 text-xs" /> {{ t("git.page.abortRebase") }}
          </Button>
        </div>
      </div>
      ...
      >
        <i class="pi pi-exclamation-triangle text-amber-600" />
        <span class="font-semibold text-amber-800">{{ t("git.page.cherryPickInProgress") }}</span>
        <span class="text-amber-700">{{ t("git.page.resolveConflictHint") }}</span>
        <div class="ml-auto flex gap-2">
          <Button size="small" outlined severity="secondary" @click="openConflictDialog">
            <i class="pi pi-wrench mr-1.5 text-xs" /> {{ t("git.page.resolveConflict") }}
          ...
          <Button size="small" outlined severity="danger" :disabled="!!git.busyMessage.value" @click="git.cherryPickAbort()">
            <i class="pi pi-times mr-1.5 text-xs" /> {{ t("git.page.abortCherryPick") }}
          </Button>
        </div>
      </div>
      ...
      >
        <i class="pi pi-exclamation-triangle text-amber-600" />
        <span class="font-semibold text-amber-800">{{ t("git.page.mergeInProgress") }}</span>
        <span class="text-amber-700">{{ t("git.page.resolveConflictHintMerge") }}</span>
        <div class="ml-auto flex gap-2">
          <Button size="small" outlined severity="secondary" @click="openConflictDialog">
            <i class="pi pi-wrench mr-1.5 text-xs" /> {{ t("git.page.resolveConflict") }}
          </Button>
          <Button size="small" outlined severity="danger" :disabled="!!git.busyMessage.value" @click="git.mergeAbort()">
            <i class="pi pi-times mr-1.5 text-xs" /> {{ t("git.page.abortMerge") }}
          </Button>
        </div>
      </div>
      ...
      >
        ...
        <div>
          <p class="text-base font-semibold text-ink">{{ t("git.page.noRepoOpen") }}</p>
          <p class="mt-1 text-sm text-muted">{{ t("git.page.noRepoOpenHint") }}</p>
        </div>
        <div class="flex gap-2">
          <Button size="small" @click="git.addRepoFromDialog()">
            <i class="pi pi-folder-open mr-1.5" /> {{ t("git.page.addLocalRepo") }}
          </Button>
          <Button size="small" outlined severity="secondary" @click="cloneDialogVisible = true">
            <i class="pi pi-cloud-download mr-1.5" /> {{ t("git.page.cloneFromUrl") }}
          </Button>
        </div>
      </div>
          ...
          >
            ...
            <div class="min-h-0 flex-1 overflow-y-auto">
              ...
              <div v-if="git.staged.value.length" class="border-b border-divider">
                <div class="sticky top-0 flex items-center gap-2 bg-canvas px-3 py-1.5">
                  ...
                  <span class="text-xs font-bold uppercase tracking-wide text-muted">{{ t("git.page.staged") }}</span>
              ...
              <div>
                <div class="sticky top-0 flex items-center gap-2 bg-canvas px-3 py-1.5">
                  <Checkbox
                    ...
                    @change="git.stageAll()"
                  />
                  <span class="text-xs font-bold uppercase tracking-wide text-muted">{{ t("git.page.changes") }}</span>
                  ...
                  <div class="ml-auto flex items-center gap-2">
                    <button
                      ...
                      @click="askDiscard(git.unstaged.value.map((x: GitFileChange) => x.path), t('git.page.discardAllLabel'))"
                    >
                      {{ t("git.page.discardAll") }}
                    </button>
                    <button
                      v-if="git.unstaged.value.length"
                ...
                >
                  ...
                  <i
                    ...
                    :title="t('git.page.discardChange')"
            ...
            <div class="border-t border-divider bg-canvas p-3">
              <Textarea
                ...
                :rows="3"
                auto-resize
                :placeholder="t('git.page.commitMessagePlaceholder')"
              ...
              <div class="mt-2 flex items-center gap-2">
                ...
                <Button
                  ...
                  outlined
                  severity="secondary"
                  :title="t('git.page.viewStashList')"
                  @click="openStashList"
                >
                  <i class="pi pi-list" />
              ...
              <div v-if="git.staged.value.length" class="mt-1.5 text-[11px] text-muted">
                {{ t("git.page.filesStagedHint", { count: git.staged.value.length }) }}
              </div>
            </div>
          </div>
            ...
            <div v-if="git.commitDetail.value" class="border-b border-divider bg-canvas px-4 py-2.5">
              <div class="flex items-start gap-2">
                ...
                <button
                  ...
                  :title="detailExpanded ? t('git.page.collapse') : t('git.page.expand')"
    ...
    <div
      ...
      @click.stop
    >
      <button class="ctx-menu-item" @click="git.copyText(absPath(fileMenu.rel), t('git.page.copyLabel.path')); closeFileMenu()">
        <i class="pi pi-copy text-xs" /> {{ t("git.page.copyPath") }}
      </button>
      <button class="ctx-menu-item" @click="git.copyText(relPath(fileMenu.rel), t('git.page.copyLabel.relativePath')); closeFileMenu()">
        <i class="pi pi-copy text-xs" /> {{ t("git.page.copyRelativePath") }}
      </button>
      <button class="ctx-menu-item" @click="git.showInFolder(absPath(fileMenu.rel)); closeFileMenu()">
        <i class="pi pi-folder-open text-xs" /> {{ t("git.page.showInFolder") }}
      </button>
      <div class="my-1 border-t border-divider" />
      <button class="ctx-menu-item" @click="openBlameDialog(fileMenu.rel)">
        <i class="pi pi-user-edit text-xs" /> {{ t("git.page.viewBlame") }}
      </button>
    </div>
    ...
    >
      <button v-if="isTopCommit(commitMenu.commit)" class="ctx-menu-item" @click="handleAmendCommit(commitMenu.commit); closeCommitMenu()">
        <i class="pi pi-pencil text-xs" /> {{ t("git.page.commitMenu.amend") }}
      </button>
      ...
      <button class="ctx-menu-item" @click="openTagDialog({ hash: commitMenu.commit.hash, label: commitMenu.commit.short_hash + ' — ' + commitMenu.commit.subject })">
        <i class="pi pi-tag text-xs" /> {{ t("git.page.commitMenu.createTag") }}
      </button>
      <div class="my-1 border-t border-divider" />
      <button class="ctx-menu-item" @click="git.resetTo(commitMenu.commit.hash, 'mixed'); closeCommitMenu()">
        <i class="pi pi-history text-xs" /> {{ t("git.page.commitMenu.reset") }}
      </button>
      <button class="ctx-menu-item-danger" @click="askResetHard(commitMenu.commit); closeCommitMenu()">
        <i class="pi pi-exclamation-triangle text-xs" /> {{ t("git.page.commitMenu.resetHard") }}
      </button>
      <div class="my-1 border-t border-divider" />
      <button class="ctx-menu-item" @click="git.copyText(commitMenu.commit.hash, t('git.page.copyLabel.sha')); closeCommitMenu()">
        <i class="pi pi-copy text-xs" /> {{ t("git.page.commitMenu.copySha") }}
      </button>
      <button class="ctx-menu-item" @click="git.copyText(commitMenu.commit.subject, t('git.page.copyLabel.commitMessage')); closeCommitMenu()">
        <i class="pi pi-copy text-xs" /> {{ t("git.page.commitMenu.copyMessage") }}
      </button>
    </div>
  </section>
</template>
