...
import type { GitBlameLine } from "@/models/git";
...
<template>
  <Dialog
    ...
    modal
    maximizable
    :header="t('git.dialogs.blame.title')"
    ...
    <div class="flex h-[70vh] max-h-[80vh] min-h-[420px] gap-2">
      ...
      <div class="flex min-w-0 flex-1 flex-col overflow-hidden rounded-md border border-divider">
        <div class="flex items-center gap-2 border-b border-divider bg-canvas px-2 py-1 text-[11px] text-muted">
          ...
          <span v-if="git.blameLines.value.length" class="ml-auto shrink-0">{{ t('git.dialogs.blame.lineCount', { count: git.blameLines.value.length }) }}</span>
        </div>
        <div class="min-h-0 flex-1 overflow-auto">
          <div v-if="git.blameLoading.value" class="p-6 text-center text-sm text-muted">
            <i class="pi pi-spinner pi-spin mr-1.5" /> {{ t('git.dialogs.blame.analyzing') }}
          </div>
          <div v-else-if="!rows.length" class="p-6 text-center text-sm text-muted">
            {{ t('git.dialogs.blame.noData') }}
          </div>
          <button
            v-for="(row, i) in rows"
            v-else
          ...
          >
            ...
            <span v-if="row.groupStart" class="truncate">
              <template v-if="isUncommitted(row.line.hash)">
                <span class="italic text-muted">{{ t('git.dialogs.blame.uncommitted') }}</span>
              </template>
              <template v-else>
                <span class="font-semibold text-ink">{{ row.line.author_name || t('git.dialogs.blame.unknownAuthor') }}</span>
      ...
      <DialogFooter :cancel-label="t('git.dialogs.blame.close')" cancel-icon="pi pi-times" cancel-severity="warn" hide-confirm @cancel="visible = false">
        <template #extra>
          ...
          <Button size="small" outlined :disabled="!git.blameSelectedHash.value" @click="copySha">
            <i class="pi pi-copy mr-1.5" /> {{ t('git.dialogs.blame.copySha') }}
          </Button>
          <Button size="small" outlined :disabled="!git.blameSelectedHash.value" @click="openInHistory">
            <i class="pi pi-history mr-1.5" /> {{ t('git.dialogs.blame.viewInHistory') }}
          </Button>
        </template>
      </DialogFooter>
    </template>
