...
<template>
  <Dialog
    ...
    modal
    maximizable
    :header="t('git.dialogs.worktreeList.title')"
    :style="{ width: '620px' }"
  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-[11px] text-muted">
            ...
            <span v-if="w.is_current" class="ml-1 text-brand">{{ t('git.dialogs.worktreeList.currentlyOpen') }}</span>
          </p>
        </div>
        <Button
          v-if="!w.is_current"
      ...
      <div v-if="!git.worktrees.value.length" class="p-4 text-center text-sm text-muted">
        {{ t('git.dialogs.worktreeList.emptyMessage') }}
      </div>
    </div>
    <template #footer>
      <DialogFooter
