...
const allBranchRefs = computed(() =>
  props.git.branches.value
    ...
    .map((b) => ({
      label: b.is_remote ? t("git.dialogs.compare.remoteBranchLabel", { name: b.name }) : b.name,
      value: b.name,
    })),
);
...
<template>
  <Dialog
    ...
    modal
    maximizable
    :header="t('git.dialogs.compare.title')"
    :style="{ width: '840px' }"
  ...
  >
    <div class="flex flex-col gap-3">
      ...
      <p v-if="cmpBase === cmpHead" class="text-xs text-amber-600">
        {{ t('git.dialogs.compare.sameBranchWarning') }}
      </p>

      <div v-if="git.comparison.value" class="flex flex-wrap items-center gap-2 text-xs">
        <span class="badge-success">
          {{ t('git.dialogs.compare.aheadCount', { count: git.comparison.value.ahead }) }}
        </span>
        <span v-if="git.comparison.value.behind" class="badge-info">
          {{ t('git.dialogs.compare.behindCount', { count: git.comparison.value.behind }) }}
        </span>
        <span class="text-muted">{{ t('git.dialogs.compare.filesChangedCount', { count: git.comparison.value.files.length }) }}</span>
      </div>
      ...
      >
        ...
        <div class="flex w-64 shrink-0 flex-col overflow-hidden rounded-md border border-divider">
          <div class="border-b border-divider bg-canvas px-2 py-1 text-[11px] font-bold uppercase tracking-wide text-muted">
            {{ t('git.dialogs.compare.commits', { count: git.comparison.value.commits.length }) }}
          </div>
    ...
    <template #footer>
      ...
      >
        <template #extra>
          ...
          >
            {{ t('git.dialogs.compare.noRemoteWarning') }}
          </span>
        </template>
      </DialogFooter>
    </template>
