...
onBeforeUnmount(() => {
  // Cố ý KHÔNG gọi `term.dispose()` ở đây — các phiên terminal (vd. do màn hình
  // Git mở để chạy AI agent) cần tiếp tục chạy nền kể cả khi rời trang này.
  // Chỉ dọn dẹp listener DOM cục bộ của chính trang (kéo giãn cột cây thư mục).
  if (activeTreeResizeMove) document.removeEventListener("mousemove", activeTreeResizeMove);
  document.removeEventListener("mouseup", endResizeTree);
});
</script>
